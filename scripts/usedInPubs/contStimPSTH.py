...
def plotFRTSKDESmall(dataXL, outBase):
    """
    Estimates the firing rate profile from the spiking data in <dataXL> using AdaptiveKDE[1][2], plots and saves it.
    :param dataXL: string, path of the excel file generated by the function "saveContStimPSTH".
    :param outBase: string, output file will be saved as "<outBase>.png"
...
def plotKDEAndIndNrns(dataXL, outBase):
    """
    Estimates the firing rate profiles from the spiking data in <dataXL> using AdaptiveKDE[1][2]. Two kinds of
    profiles are calculated, from spiking data of individual neurons and spiking data of all neurons put together.
    The profiles of individual neurons are plotted against the background of the pooled profile.
    :param dataXL: string, path of the excel file generated by the function "saveContStimPSTH".
    :param outBase: string, output file will be saved as "<outBase>.png"
    :return:
    ...
    if nLS > 1:
        print("Warning! The data in {} belongs to multiple labor states. This will be ignored and all data will be "
              "pooled")
    else:
        if dataDF[mdFN["laborState"]].unique()[0] == "Newly Emerged":
            palette = "r"
        else:
            palette = "b"

    allSpikeTimes = dataDF["Spike Time (s)"]
    trialExp = dataDF.apply(lambda x: "{}{}".format(x[mdFN["expID"]], x[mdFN["trialName"]]), axis=1)
    nTrials = trialExp.unique().size
    nSpikes = dataDF.shape[0]
    res = ssvkernel(x=allSpikeTimes.values, tin=tEst, WinFunc="Gauss")

    fr = res[0] * nSpikes / nTrials
    confInt95 = res[5][0] * nSpikes / nTrials

    ax1.plot(tEst, fr, color=palette, ls="-", marker="None")
    ax1.fill_between(tEst, fr - 0.5 * confInt95, fr + 0.5 * confInt95, facecolors=palette, interpolate=True,
                     alpha=0.2)

    for expInd, (expID, expDF) in enumerate(dataDF.groupby(mdFN["expID"])):
        spikeData = expDF
        spikeTimes = spikeData["Spike Time (s)"]
        ...
        # optW = res[2]
        col = 'g'
        if expID in ["130408-1LY", "130501-2Rh"]:
            col = 'm'
        ax1.plot(tEst, fr, color=col, ls='-', marker=None)
...
if __name__ == "__main__":
    ...
    assert len(sys.argv) == 4, "Improper Usage! Please use as\n" \
                               ...
                               "<output base>\n" \
                               "python {currFile} plotFRTSKDESmall <data excel file> " \
                               "<output base>\n" \
                               "python {currFile} plotKDEAndIndNrns <data excel file> " \
    ...
    elif sys.argv[1] == "plotFRTSKDESmall":
        plotFRTSKDESmall(*sys.argv[2:])
    elif sys.argv[1] == "plotKDEAndIndNrns":
        plotKDEAndIndNrns(*sys.argv[2:])
